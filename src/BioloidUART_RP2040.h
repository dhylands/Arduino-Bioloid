...
class BioloidUART {
 public:
    ...
    //! @returns uint8_t containing the number of bytes available for reading.
    uint8_t available() { return this->serial_uart->available(); }

    //! Reads a byte from the UART.
    //! @returns uint8_t containg the byte that was read.
